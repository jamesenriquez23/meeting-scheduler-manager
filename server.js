  ...
  try {
    ...
    db.run(
      ...
      function (err) {
        ...
        // Non-blocking trigger: Send email notification to admin upon registration
...
app.post('/api/schedule/generate', authenticateToken, (req, res) => {
  ...
  db.all(`SELECT * FROM brothers WHERE user_id = ?`, [req.user.id], (err, brothers) => {
    if (err || brothers.length === 0) {
      return res.status(400).json({ error: 'No brothers available in roster to generate schedule.' });
    }

    const dates = [`07.${String(month).padStart(2, '0')}.${year}`, `14.${String(month).padStart(2, '0')}.${year}`, `21.${String(month).padStart(2, '0')}.${year}`, `28.${String(month).padStart(2, '0')}.${year}`];
    ...
    db.run(`DELETE FROM schedules WHERE user_id = ? AND month = ? AND year = ?`, [req.user.id, month, year], () => {
      ...
      dates.forEach((dateStr) => {
        const reader = brothers.find((b) => b.can_wt_reader)?.name || 'N/A';
        const chairman = brothers.find((b) => b.can_chairman)?.name || 'N/A';
        const attendant = brothers.find((b) => b.can_attendants)?.name || 'N/A';
        const microver = brothers.find((b) => b.can_microvers)?.name || 'N/A';
        const av = brothers.find((b) => b.can_av)?.name || 'N/A';

        stmt.run(req.user.id, dateStr, reader, chairman, attendant, microver, av, month, year);
      });

      stmt.finalize(() => {
        db.all(`SELECT * FROM schedules WHERE user_id = ?`, [req.user.id], (fetchErr, rows) => {
